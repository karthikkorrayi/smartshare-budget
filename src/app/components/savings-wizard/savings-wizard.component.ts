...
@Component({
  ...
  templateUrl: './savings-wizard.component.html',
  styleUrl: './savings-wizard.component.scss'
})
export class SavingsWizardComponent {
    ...
    if (id) {
      ...
      if (g && g.type==='savings') {
        ...
        this.form.patchValue({
          title:g.title, target:g.price, months:g.desiredMonths ?? g.monthsRequired,
  ...
  addExpense(name='', amount:number|null=null){ 
    this.expensesFA.push(this.fb.group({ 
      name:[name], 
  ...
  back(){ 
    if (this.step>0) 
      this.step--; 
    else if(this.isEdit && this.editingId) 
      this.router.navigate(['/savings', this.editingId]); 
    }
  ...
  finish(){
    ...
    const monthly=Math.ceil(Number(v.target) / Number(v.months));

    const startISO=this.monthISONow();
    const schedule=this.planner.buildFlat(Number(v.target), startISO, monthly);

    const plan={
      id: this.isEdit ? this.editingId! : ('savings-' + crypto.randomUUID()),
      type:'savings',
      title:String(v.title),
      price:Number(v.target),
      desiredMonths:Number(v.months),
      incomeMin, incomeMax,
      expenses,
    ...
    this.router.navigate(['/savings', plan.id]);
  }
